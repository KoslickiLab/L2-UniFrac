...
def generate_diffab(regions, region_averages, Tint, lint, nodes_in_order, output, thresh, maxDisp):
	for i in range(len(region_averages)):
		for j in range(len(region_averages)):
			if i < j:
				...
				fig = L2U.plot_diffab(nodes_in_order, DifferentialAbundance, regions[i], regions[j], plot_zeros=False, thresh=thresh, show=False, maxDisp=maxDisp)
