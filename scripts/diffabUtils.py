...
def generate_diffab(regions, region_averages, Tint, lint, nodes_in_order, taxonomy_in_order, output, thresh, maxDisp, includeTemp, L, include_tmp_diffab=False):
	if include_tmp_diffab:
		for i in range(len(region_averages)):
			tax_mass_dict = {}
			for k in range(len(taxonomy_in_order)):
				if taxonomy_in_order[k] not in tax_mass_dict:
					tax_mass_dict[taxonomy_in_order[k]] = 0
				tax_mass_dict[taxonomy_in_order[k]] += region_averages[regions[i]][k]
			for k in range(len(taxonomy_in_order)):
				region_averages[regions[i]][k] = tax_mass_dict[taxonomy_in_order[k]]
				tax_mass_dict[taxonomy_in_order[k]] = 0
	if L == 1:
		for i in range(len(region_averages)):
