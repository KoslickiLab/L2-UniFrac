...
def generate_group_pcoa(biom_file, tree_file, metadata_file, tax_file, verbose, threads, intermediate_store, preprocessed_use, unifrac_code, output_file):
	...
	if unifrac_code == 1 or unifrac_code == 2:
		_, _, _, _, _, L1_distance_matrix, _ = avg.compute_averages(L1_preprocessed, biom_file, tree_file, metadata_file, tax_file, 'reports/' + str(output_file) + '_avg_report.csv', 2)
	...
	if unifrac_code == 0 or unifrac_code == 1:
		_, _, _, _, _, L2_distance_matrix, _ = avg.compute_averages(L2_preprocessed, biom_file, tree_file, metadata_file, tax_file, 'reports/' + str(output_file) + '_avg_report.csv', 0)
